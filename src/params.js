...
// <- PONTO CRÍTICO: manter esta função em linhas curtas e ASCII
export function applyCatalogEdits(){
  try{
    if(!CAT_EDITS) return;
    const base = (window.DATA||[]);
    window.DATA = base.map(d=>{
      const k = d.marca + '\n' + d.modelo;
      const p = CAT_EDITS[k];
      if(!p) return d;
      const out = Object.assign({}, d);
      if(p.preco_net!=null) out.preco_net = p.preco_net;
      if(p.wlpt_km!=null) out.wlpt_km = p.wlpt_km;
      if(p.cap!=null) out.cap = p.cap;
      if(p.cons!=null) out.cons = p.cons;
      if(p.pdc!=null) out.pdc = p.pdc;
      if(p.mala!=null) out.mala = p.mala;
      return out;
    });
  }catch(e){}
}
...
export function addCustom(){
  const d={
    marca:q('cMarca').value||'',
    modelo:q('cModelo').value||'',
    preco_net:+q('cPreco').value||0,
    wlpt_km:+q('cWLTP').value||0,
    cap:+q('cCap').value||0,
    cons:+q('cCons').value||0,
    mala:+q('cMala').value||0,
    pdc:+q('cPdc').value||120
  };
  if(!d.marca||!d.modelo||!d.preco_net||!d.wlpt_km||!d.cap||!d.cons){
    alert('Preencher Marca, Modelo, Preço, WLTP, Cap e Consumo.');
    return;
  }
  (window.CUSTOM = window.CUSTOM||[]).push(d);
